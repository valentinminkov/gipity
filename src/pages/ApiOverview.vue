<template>
  <q-page padding>
    <q-table :rows="tableData" :columns="columns" row-key="id" virtual-scroll binary-state-sort flat bordered
      :rows-per-page-options="[10, tableData?.length]" title="Models Overview">
...
const tableData = ref(formatTableData(dummyData.data));
...
const setModels = async () => {
  ...
  tableData.value = formatTableData(res?.data);
}
